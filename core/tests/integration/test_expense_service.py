...
def test_remove_expense_reduces_total():
    ...
    e1 = service.create_expense("Libro", 20, "", date.today())
...
def test_update_expense_partial_fields():
    ...
    expense_init = service.create_expense("Camiseta", 15, "Ropa", date.today())

    service.update_expense(expense_id=expense_init.id, amount=18)

    expense_updated = service.list_expenses()[0]
    assert expense_updated.title == "Camiseta"
    assert expense_updated.amount == 18
    assert expense_updated.description == "Ropa"
...
def test_total_amount_after_removal():
    ...
    e1 = service.create_expense("Cursos", 30, "", date.today())
